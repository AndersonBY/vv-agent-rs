...
| Canonical 15-tool surface | `crates/vv-agent/src/tools/registry/defaults.rs`, `crates/vv-agent/src/constants/tool_names.rs`, `crates/vv-agent/src/tools/executor.rs`; `crates/vv-agent/tests/parity_evidence_manifests.rs`, `crates/vv-agent/tests/tool_schema_contract.rs`, `crates/vv-agent/tests/builtin_tool_behavior_contract.rs` |
| Contract 21 Bash management receipts, original execution deadline, task/workspace ownership and confirmed stop | `crates/vv-agent/src/runtime/background_sessions/`, `crates/vv-agent/src/runtime/processes/`, `crates/vv-agent/src/tools/handlers/bash/`, `crates/vv-agent/src/tools/handlers/background.rs`; `crates/vv-agent/tests/bash_process_management.rs`, `crates/vv-agent/tests/bash_tools.rs` |
...
The ignored SQLite probe in `runtime/checkpoint_resume/persistence/tests.rs` exchanges
real planner output with Python and resumes the same operation through each
recovery controller. Central CI supplies both databases and runs it explicitly.
`canonical_unknown_journal_recovers_once_through_controller` consumes the central
