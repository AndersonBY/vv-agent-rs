...
#[tokio::test]
async fn initialize_matches_the_cross_language_contract_and_does_not_advertise_missing_runtime() {
...
#[tokio::test]
async fn thread_start_and_unsubscribe_use_cross_language_payloads() {
...
#[tokio::test]
async fn turn_controls_match_cross_language_errors_without_an_active_turn() {
    let (mut processor, mut outgoing) = processor_with_runtime(Vec::new());
    let connection_id = ConnectionId::new(1);
    initialize(&mut processor, &mut outgoing, connection_id).await;
...
#[tokio::test]
async fn turn_completion_keeps_cross_language_terminal_fields() {
...
async fn steer_updates_active_turn_and_follow_up_starts_next_turn() {
    ...
    let slow_tool = FunctionTool::builder("slow_tool")
        .description("Wait for the cross-language contract test gate.")
...
#[test]
fn tool_started_event_emits_real_tool_delta_with_cross_language_item_fields() {
...
#[test]
fn approval_request_matches_the_cross_language_payload() {
...
fn initialize_request(id: i64) -> JsonRpcMessage {
    request(
        ...
        "initialize",
        json!({
            "clientInfo": {"name": "cross-language-fixture"},
