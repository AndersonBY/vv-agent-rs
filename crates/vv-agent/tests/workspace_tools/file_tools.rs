...
#[test]
fn edit_file_large_document_consecutive_edits_keep_compact_receipts_and_current_baseline() {
    let workspace = tempfile::tempdir().expect("workspace");
    let registry = build_default_registry();
    let mut context = ToolContext::new(workspace.path());
    let target = workspace.path().join("report.md");
    let before = "重复正文 references / 中文来源\n".repeat(4_000)
        + &(0..7)
            .map(|i| format!("source{i}/target"))
            .collect::<Vec<_>>()
            .join("\n");
    std::fs::write(&target, &before).expect("file");
    let read = registry
        .execute(
            &ToolCall::new(
                "read_report",
    ...
    for i in 0..7 {
        let result = registry
            .execute(
                &ToolCall::new(
                    format!("edit_report_{i}"),
                    "edit_file",
                    BTreeMap::from([
                        ("path".to_string(), json!("report.md")),
                        ("old_string".to_string(), json!(format!("source{i}/target"))),
                        (
                            "new_string".to_string(),
                            json!(format!("source{i} / target")),
                        ),
                    ]),
                ),
                &mut context,
        ...
        assert_eq!(result.status, ToolResultStatus::Success);
        assert_eq!(
            serde_json::from_str::<Value>(&result.content).expect("receipt"),
            json!({"ok": true, "path": "report.md", "replaced_count": 1})
        );
        assert_eq!(
            json!(result.metadata),
            json!({"changed_files": ["report.md"], "operation": "edit_file", "line_ending": "lf"})
        );
        expected = expected.replace(&format!("source{i}/target"), &format!("source{i} / target"));
    }
    assert!(started.elapsed() < std::time::Duration::from_secs(5));
    assert_eq!(std::fs::read_to_string(&target).expect("file"), expected);

    std::fs::write(&target, format!("{expected}\nexternal change")).expect("file");
    let rejected = registry
        .execute(
            &ToolCall::new(
                ...
                BTreeMap::from([
                    ("path".to_string(), json!("report.md")),
                    ("old_string".to_string(), json!("source0")),
                    ("new_string".to_string(), json!("updated")),
                ]),
            ),
            &mut context,
        )
