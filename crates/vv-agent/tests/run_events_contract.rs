use serde_json::json;
use vv_agent::events::ApprovalAction;
use vv_agent::{AgentErrorPayload, AgentStatus, RunEvent, RunEventPayload};

const PARITY_FIXTURE: &str = include_str!("fixtures/parity/run_events.jsonl");
const BUDGET_FIXTURE: &str = include_str!("fixtures/parity/budget_events.jsonl");

#[test]
fn run_event_has_identity_trace_session_and_timing() {
    let event = RunEvent::run_started("run_1", "trace_1", "assistant", "hello")
        .with_session_id("session_1")
        .with_metadata("source", json!("test"));

    assert_eq!(event.version().as_str(), "v5");
    assert!(event.event_id().as_str().starts_with("evt_"));
    assert_eq!(event.run_id(), "run_1");
    assert_eq!(event.trace_id(), "trace_1");
    assert_eq!(event.session_id(), Some("session_1"));
    assert!(event.created_at() > 0.0);
    assert_eq!(event.agent_name(), Some("assistant"));
    assert!(matches!(event.payload(), RunEventPayload::RunStarted { input } if input == "hello"));

    let encoded = serde_json::to_value(&event).expect("serialize");
    assert_eq!(encoded["version"], "v5");
    assert_eq!(encoded["type"], "run_started");
    assert_eq!(encoded["run_id"], "run_1");
    assert_eq!(encoded["trace_id"], "trace_1");
    assert_eq!(encoded["session_id"], "session_1");
    assert_eq!(encoded["input"], "hello");

    let decoded: RunEvent = serde_json::from_value(encoded).expect("deserialize");
    assert_eq!(decoded.run_id(), "run_1");
}

#[test]
fn run_event_v5_rejects_stale_and_future_discriminators() {
    let current = serde_json::to_value(RunEvent::run_started(
        "run_version",
        "trace_version",
        "assistant",
        "hello",
    ))
    .expect("current event");

    for version in ["v4", "v6"] {
        let mut stale = current.clone();
        stale["version"] = json!(version);
        let error =
            serde_json::from_value::<RunEvent>(stale).expect_err("non-current version rejected");
        assert!(error.to_string().contains("unsupported run event version"));
    }
}

#[test]
fn child_event_records_parent_run_and_parent_event() {
    let event = RunEvent::tool_call_started(
        "run_child",
        "trace_1",
        "researcher",
        2,
        "call_1",
        "read_file",
        json!({"path": "README.md"}),
    )
    .with_parent_run_id("run_parent")
    .with_parent_event_id("evt_parent");

    assert_eq!(event.parent_run_id(), Some("run_parent"));
    assert_eq!(event.parent_event_id(), Some("evt_parent"));
    assert_eq!(event.cycle_index(), Some(2));
}

#[test]
fn run_completed_payload_round_trips_status() {
    let event = RunEvent::run_completed("run_1", "trace_1", "assistant", AgentStatus::Completed);
    let value = serde_json::to_value(&event).expect("serialize");
    assert_eq!(value["type"], "run_completed");
    let decoded: RunEvent = serde_json::from_value(value).expect("deserialize");
    assert!(matches!(
        decoded.payload(),
        RunEventPayload::RunCompleted { status } if *status == AgentStatus::Completed
    ));
}

#[test]
fn typed_stream_wire_requires_positive_cycle_index() {
    let error = serde_json::from_value::<RunEvent>(json!({
        "version": "v5",
        "type": "assistant_delta",
        "event_id": "evt_stream",
        "run_id": "run_stream",
        "trace_id": "trace_stream",
        "created_at": 1,
        "delta": "hello"
    }))
    .expect_err("typed stream event without a cycle must fail");

    assert!(error
        .to_string()
        .contains("missing required fields: cycle_index"));
}

#[test]
fn run_events_parity_fixture_has_stable_bytes_and_round_trips() {
    let expected_types = [
        "run_started",
        "agent_started",
        "cycle_started",
        "model_call_started",
        "model_call_failed",
        "model_call_started",
        "assistant_delta",
        "reasoning_delta",
        "model_tool_call_started",
        "model_tool_call_progress",
        "model_call_completed",
        "tool_call_planned",
        "tool_call_started",
        "tool_call_completed",
        "approval_requested",
        "approval_resolved",
        "memory_compact_started",
        "model_call_started",
        "model_call_completed",
        "memory_compact_completed",
        "sub_run_started",
        "sub_run_completed",
        "handoff_started",
        "handoff_completed",
        "session_persisted",
        "run_state_changed",
        "run_state_changed",
        "diagnostic",
        "run_completed",
        "run_failed",
        "run_cancelled",
        "cycle_aborted",
        "operation_ambiguous",
        "cycle_aborted",
        "run_state_changed",
        "run_failed",
        "operation_ambiguous",
        "cycle_aborted",
        "run_state_changed",
        "run_failed",
        "tool_call_deferred",
        "tool_call_completed",
        "budget_snapshot",
        "budget_exhausted",
        "checkpoint_created",
        "checkpoint_resumed",
        "operation_replayed",
        "operation_ambiguous",
        "model_retry_duplicate_risk",
        "reconciliation_resolved",
        "reconciliation_required",
        "host_interaction_requested",
        "host_interaction_response_consumed",
    ];
    let mut actual_types = Vec::new();

    for line in PARITY_FIXTURE.lines() {
        let expected: serde_json::Value = serde_json::from_str(line).expect("fixture JSON");
        let event: RunEvent = serde_json::from_str(line).expect("deserialize fixture event");
        let encoded = serde_json::to_value(&event).expect("serialize fixture event");

        actual_types.push(expected["type"].as_str().expect("event type").to_string());
        if event.run_id() == "run_parity" {
            assert_eq!(
                event.event_id().as_str(),
                expected["event_id"].as_str().expect("event id")
            );
            assert_eq!(event.run_id(), "run_parity");
            assert_eq!(event.trace_id(), "trace_parity");
            assert_eq!(event.created_at(), 123.456789);
        }
        assert_eq!(encoded, expected);
    }

    assert_eq!(actual_types, expected_types);
}

#[test]
fn memory_lifecycle_events_reject_missing_current_fields() {
    for incomplete in [
        json!({
            "version": "v5",
            "type": "memory_compact_started",
            "event_id": "evt_incomplete_started",
            "run_id": "run_incomplete",
            "trace_id": "trace_incomplete",
            "created_at": 1,
            "cycle_index": 2,
            "agent_name": "assistant",
            "message_count": 8,
            "estimated_tokens": 120
        }),
        json!({
            "version": "v5",
            "type": "memory_compact_completed",
            "event_id": "evt_incomplete_completed",
            "run_id": "run_incomplete",
            "trace_id": "trace_incomplete",
            "created_at": 1,
            "cycle_index": 2,
            "agent_name": "assistant",
            "before_count": 8,
            "after_count": 3,
            "summary_tokens": 20
        }),
    ] {
        let error = serde_json::from_value::<RunEvent>(incomplete)
            .expect_err("memory event missing current fields must fail");
        assert!(error.to_string().contains("missing required fields"));
    }
}

#[test]
fn budget_events_fixture_has_stable_bytes_and_round_trips() {
    let expected_types = [
        "budget_snapshot",
        "budget_exhausted",
        "run_failed",
        "run_completed",
    ];
    let mut actual_types = Vec::new();

    for line in BUDGET_FIXTURE.lines() {
        let expected: serde_json::Value = serde_json::from_str(line).expect("fixture JSON");
        let event: RunEvent = serde_json::from_str(line).expect("deserialize budget event");
        let encoded = serde_json::to_value(&event).expect("serialize budget event");

        actual_types.push(expected["type"].as_str().expect("event type").to_string());
        assert_eq!(encoded, expected);
    }

    assert_eq!(actual_types, expected_types);
}

#[test]
fn approval_resolved_action_is_the_only_wire_decision() {
    let cases = [
        ("allow", ApprovalAction::Allow),
        ("allow_session", ApprovalAction::AllowSession),
        ("deny", ApprovalAction::Deny),
        ("timeout", ApprovalAction::Timeout),
    ];

    for (wire_action, expected_action) in cases {
        let event: RunEvent = serde_json::from_value(json!({
            "version": "v5",
            "type": "approval_resolved",
            "event_id": "evt_approval",
            "run_id": "run_approval",
            "trace_id": "trace_approval",
            "created_at": 123.456789,
            "request_id": "req_1",
            "tool_name": "shell",
            "tool_call_id": "call_1",
            "action": wire_action
        }))
        .expect("deserialize canonical approval action");

        assert_eq!(event.approval_action(), Some(expected_action));
        assert!(matches!(
            event.payload(),
            RunEventPayload::ApprovalResolved { action, .. } if *action == expected_action
        ));
        let encoded = serde_json::to_value(&event).expect("serialize approval action");
        assert_eq!(encoded["action"], wire_action);
        assert!(encoded.get("approved").is_none());
        let restored: RunEvent = serde_json::from_value(encoded).expect("round-trip approval");
        assert_eq!(restored.approval_action(), Some(expected_action));
    }
}

#[test]
fn approval_resolved_rejects_superseded_approved_field() {
    let error = serde_json::from_value::<RunEvent>(json!({
        "version": "v5",
        "type": "approval_resolved",
        "event_id": "evt_approval",
        "run_id": "run_approval",
        "trace_id": "trace_approval",
        "created_at": 123.456789,
        "request_id": "req_1",
        "tool_name": "shell",
        "tool_call_id": "call_1",
        "action": "timeout",
        "approved": true
    }))
    .expect_err("superseded approved field must fail");

    assert!(error.to_string().contains("unknown fields: approved"));
}

#[test]
fn created_at_keeps_microseconds_and_rejects_superseded_milliseconds() {
    let event: RunEvent = serde_json::from_value(json!({
        "version": "v5",
        "type": "run_started",
        "event_id": "evt_current",
        "run_id": "run_current",
        "trace_id": "trace_current",
        "created_at": 123.456789,
        "input": "hello"
    }))
    .expect("deserialize current timestamp");

    assert_eq!(event.created_at(), 123.456789);
    let encoded = serde_json::to_value(event).expect("serialize seconds timestamp");
    assert_eq!(encoded["created_at"], json!(123.456789));

    let error = serde_json::from_value::<RunEvent>(json!({
        "version": "v5",
        "type": "run_started",
        "event_id": "evt_old_time",
        "run_id": "run_old_time",
        "trace_id": "trace_old_time",
        "created_at": 123.456789,
        "created_at_ms": 123456.789,
        "input": "hello"
    }))
    .expect_err("superseded timestamp field must fail");
    assert!(error.to_string().contains("unknown fields: created_at_ms"));
}

#[test]
fn empty_metadata_and_none_common_fields_are_omitted() {
    let encoded = serde_json::to_value(RunEvent::run_started(
        "run_compact",
        "trace_compact",
        "assistant",
        "hello",
    ))
    .expect("serialize compact event");

    assert!(encoded.get("metadata").is_none());
    assert!(encoded.get("session_id").is_none());
    assert!(encoded.get("cycle_index").is_none());
    assert!(encoded.get("parent_run_id").is_none());
}

#[test]
fn approval_preview_field_is_rejected() {
    let error = serde_json::from_value::<RunEvent>(json!({
        "version": "v5",
        "type": "approval_requested",
        "event_id": "evt_approval",
        "run_id": "run_approval",
        "trace_id": "trace_approval",
        "created_at": 123.456789,
        "request_id": "req_1",
        "tool_call_id": "call_1",
        "tool_name": "shell",
        "message": "Allow command?",
        "preview": "Allow command?"
    }))
    .expect_err("superseded preview field must fail");
    assert!(error.to_string().contains("unknown fields: preview"));
}

#[test]
fn run_failed_uses_string_wire_error_and_retains_typed_code_in_metadata() {
    let event = RunEvent::run_failed(
        "run_failed",
        "trace_failed",
        "assistant",
        AgentErrorPayload {
            message: "provider unavailable".to_string(),
            code: Some("provider_error".to_string()),
        },
    );

    let encoded = serde_json::to_value(event).expect("serialize failure");
    assert_eq!(encoded["error"], "provider unavailable");
    assert_eq!(encoded["metadata"]["error_code"], "provider_error");
}

#[test]
fn failed_and_cancelled_producers_emit_nullable_partial_output() {
    let failed = serde_json::to_value(RunEvent::run_failed(
        "run_failed_null_partial",
        "trace_failed_null_partial",
        "assistant",
        AgentErrorPayload::new("provider unavailable"),
    ))
    .expect("serialize failed event");
    assert!(failed
        .get("partial_output")
        .is_some_and(serde_json::Value::is_null));

    let cancelled = serde_json::to_value(RunEvent::new(
        "run_cancelled_null_partial",
        "trace_cancelled_null_partial",
        "assistant",
        None,
        RunEventPayload::RunCancelled {
            reason: "cancelled".to_string(),
        },
    ))
    .expect("serialize cancelled event");
    assert!(cancelled
        .get("partial_output")
        .is_some_and(serde_json::Value::is_null));
}

#[test]
fn live_cancel_requires_top_level_typed_transition() {
    let mut event = json!({
        "version": "v5",
        "type": "run_state_changed",
        "event_id": "evt_cancel",
        "run_id": "run_cancel",
        "trace_id": "trace_cancel",
        "created_at": 1,
        "state": "running",
        "cancel_requested": {"from": false, "to": true}
    });
    serde_json::from_value::<RunEvent>(event.clone()).expect("typed live cancel event");

    event["metadata"] = json!({"cancel_requested": {"from": false, "to": true}});
    event
        .as_object_mut()
        .expect("event object")
        .remove("cancel_requested");
    let error = serde_json::from_value::<RunEvent>(event).expect_err("metadata carrier rejected");
    assert!(error.to_string().contains("top-level typed field"));

    let mut malformed = json!({
        "version": "v5",
        "type": "run_state_changed",
        "event_id": "evt_cancel_malformed",
        "run_id": "run_cancel",
        "trace_id": "trace_cancel",
        "created_at": 1,
        "state": "running",
        "cancel_requested": {"from": false}
    });
    assert!(serde_json::from_value::<RunEvent>(malformed.take()).is_err());
}
