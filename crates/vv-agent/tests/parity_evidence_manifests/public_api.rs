fn compile_rust_member(surface: &str, target: &str, name: &str, kind: &str) {
    match (surface, target, kind) {
        ...
        ("tool_context", "vv_agent::ToolContext", "method") => {
            compile_methods!(name, vv_agent::ToolContext, [defer])
        }
...
fn public_api_manifest_compiles_real_rust_exports() {
    ...
    assert_eq!(fixture["contract"], "vv-agent-public-api-v7");
    ...
    assert_eq!(capability_ids.len(), 178);
