...
| `02_agent_profiles` | 带命名 Agent profile 的 SDK client。 |
| `03_sdk_client` | SDK one-shot run/query 和配置子 Agent。 |
| `04_session_api` | 长会话 SDK session 生命周期。 |
...
| `17_error_recovery` | SDK run 外层重试。 |
...
| `22_sdk_advanced` | streaming、thread backend 等高级 SDK 选项。 |
