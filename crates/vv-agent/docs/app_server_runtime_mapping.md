...
| Completed executor call | `tool_call_completed` maps to `item/completed`; additive payload fields are `directive`, `errorCode`, `executionStarted`, `durationMs`, and optional `toolMetadata` |
...
- `toolMetadata` is a camelCase projection of declared `ToolMetadata`; generic
  ...
  state.
- Legacy completed v1 events without additive fields retain their old payload;
  mapping must not fabricate execution, duration, directive, or error facts.
