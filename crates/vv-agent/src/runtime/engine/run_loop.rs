...
use super::memory::{
    ...
    notify_memory_before_compact, CycleMemoryCompaction, MemoryCompactCompletion,
};
use super::model_request::{
    build_model_request, cycle_stream_callback, effective_model_call_target,
};
use super::planning::block_on_engine_tool_run;
use super::run_setup::{
                ...
                *messages = request_messages;
                ...
                for (call_index, call) in response.tool_calls.iter().enumerate() {
                    ...
                    let checkpoint_plan = checkpoint.plan_tool(
                        ...
                        &patched_call,
                        || {
                            let idempotency = crate::runtime::run_definition::tool_idempotency_for(
                                &self.tool_registry,
                                &patched_call.name,
                            );
