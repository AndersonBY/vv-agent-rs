...
use crate::runtime::checkpoint_codec_v2::{checkpoint_v2_from_value, checkpoint_v2_to_value};
...
pub struct SqliteCheckpointStoreV2 {
    connection: Mutex<Connection>,
    location: PathBuf,
}

impl std::fmt::Debug for SqliteCheckpointStoreV2 {
    fn fmt(&self, formatter: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        formatter
            .debug_struct("SqliteCheckpointStoreV2")
...
impl CheckpointStoreV2 for SqliteCheckpointStoreV2 {
    fn create_checkpoint_v2(&self, checkpoint: CheckpointV2) -> CheckpointResult<bool> {
        ...
        let changed = connection
            .execute(
                r#"
                INSERT OR IGNORE INTO checkpoints_v2 (
                    checkpoint_key, schema_version, run_definition_schema, run_definition,
                    task_id, root_run_id, trace_id, run_definition_digest, resume_attempt,
                    cycle_index, status, messages, cycles, shared_state, budget_usage,
                    event_cursor, event_outbox, extension_state, model_call_journal,
    ...
    fn renew_checkpoint_claim_v2(
        ...
        let changed = transaction
            .execute(
                "UPDATE checkpoints_v2 SET lease_expires_at_ms = ?1 WHERE checkpoint_key = ?2 AND claim_token = ?3 AND lease_expires_at_ms > ?4",
                params![
    ...
    fn delete_checkpoint_v2(&self, checkpoint_key: &str) -> CheckpointResult<()> {
        self.lock()?
            .execute(
                "DELETE FROM checkpoints_v2 WHERE checkpoint_key = ?1",
    ...
    fn list_checkpoints_v2(&self) -> CheckpointResult<Vec<String>> {
        ...
        let mut statement = connection
            .prepare("SELECT checkpoint_key FROM checkpoints_v2 ORDER BY checkpoint_key")
            .map_err(sqlite_error)?;
        let rows = statement
...
impl SqlValues {
    fn from_checkpoint(checkpoint: &CheckpointV2) -> CheckpointResult<Self> {
        ...
        Ok(Self {
            ...
            unknown_fields: serde_json::to_string(&checkpoint.unknown_fields).map_err(|error| {
    ...
    let changed = transaction
        .execute(
            ...
            UPDATE checkpoints_v2 SET
                ...
                tool_journal = ?19, unknown_fields = ?20, revision = ?21,
    ...
    let mut statement = connection
        .prepare(
            ...
            SELECT checkpoint_key, schema_version, run_definition_schema, run_definition,
                   ...
                   tool_journal, unknown_fields, revision, claim_token, claimed_cycle,
                   lease_expires_at_ms, terminal_result, terminal_acknowledged
            FROM checkpoints_v2 WHERE checkpoint_key = ?1
            "#,
        )
        .map_err(sqlite_error)?;
    statement
...
fn row_to_checkpoint(row: &rusqlite::Row<'_>) -> rusqlite::Result<CheckpointResult<CheckpointV2>> {
    ...
    let unknown_fields: String = row.get(20)?;
    ...
    let result = (|| {
        let mut object = parse_object(&unknown_fields)?;
        ...
        checkpoint_v2_from_value(&Value::Object(object), MAX_EXTENSION_STATE_BYTES)
    })();
    Ok(result)
}
...
fn parse_object(raw: &str) -> CheckpointResult<Map<String, Value>> {
    parse_value(raw)?.as_object().cloned().ok_or_else(|| {
        CheckpointError::new("checkpoint_row_invalid", "unknown_fields is not an object")
    })
}
...
pub type SqliteStateStoreV2 = SqliteCheckpointStoreV2;
