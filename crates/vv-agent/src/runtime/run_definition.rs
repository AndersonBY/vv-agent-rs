...
mod projection;
mod tool_policy;
pub(crate) use frozen_task::{build_frozen_task, frozen_definition_messages};
...
pub(crate) fn build_run_definition(
    ...
    let definition = json!({
        ...
        "runtime_controls": {
            ...
            "microcompaction_policy": request.task.microcompaction_policy,
...
pub(crate) fn validate_distributed_run_definition(
    ...
    let task_matches = definition.get("prompt_bundle") == Some(&task.prompt_bundle.to_value())
        ...
        && controls.get("microcompaction_policy")
            == Some(
                &serde_json::to_value(task.microcompaction_policy)
...
fn validate_behavior_capability_refs(
    ...
    if !missing.is_empty() {
        return Err(CheckpointError::new(
            ...
            format!(
                "checkpointed runs require stable capability refs for: {}",
                missing.join(", ")
            ),
        ));
    }
...
fn take_ref(
    ...
    if required && value.is_none() {
        return Err(CheckpointError::new(
            "checkpoint_definition_unstable",
            format!("checkpointed runs require stable capability ref {slot}"),
        ));
    }
    Ok(value)
}
