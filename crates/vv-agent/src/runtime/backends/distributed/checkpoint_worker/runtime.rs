...
pub(super) fn run_agent_runtime_cycle(
    ...
    if owns_recovery {
        controller
            .refresh_authoritative()
            .map_err(|error| error.to_string())?;
        let claim_token = recovered
            .claim_token
            .as_deref()
            .ok_or_else(|| "host response execution claim missing".to_string())?;
        controller
...
pub(super) fn worker_execution_context(
    ...
    metadata.insert(
        ...
        serde_json::Value::String(envelope.root_run_id.clone()),
    );
    metadata.insert(
        "_vv_agent_trace_id".to_string(),
        serde_json::Value::String(envelope.trace_id.clone()),
    );
