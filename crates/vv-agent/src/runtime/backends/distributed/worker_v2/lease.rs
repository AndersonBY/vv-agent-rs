...
pub(super) fn run_with_checkpoint_lease_v2<T>(
    ...
    let initial_checkpoint = load_v2(store.as_ref(), &checkpoint_key)?;
    ...
    let initial = renew_checkpoint_lease_v2(
        store.as_ref(),
        &checkpoint_key,
        &claim_token,
        lease_duration_ms,
    ...
    let result = std::thread::scope(|scope| {
        ...
        let heartbeat = scope.spawn(move || {
            ...
            loop {
                ...
                match renew_checkpoint_lease_v2(
...
pub(super) fn renew_checkpoint_lease_v2(
    ...
    let renewed = store
        .renew_checkpoint_claim_v2(checkpoint_key, claim_token, lease_expires_at_ms, now_ms)
