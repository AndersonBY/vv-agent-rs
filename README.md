...
- Core runtime types expose Python-style `to_dict` / `from_dict` helpers for
  ...
  calls with `content: null`, Python-style omission of empty optional request
  fields, provider `extra_content`, and user image blocks.
...
- Python-style workspace backends: `LocalWorkspaceBackend` and
  ...
  Python-compatible UTC ISO `FileInfo.modified_at` values instead of Unix-second
  strings. They also report missing in-memory files with `NotFound` errors.
  `S3WorkspaceBackend` is backed
...
- Runtime-backed sub-agent support for `create_sub_task` / `sub_task_status`:
  ...
  such as `"true"` and `"0"` behave the same as in Python. Batch validation
  also matches Python for non-array `tasks`, non-object items, missing
  `task_description`, all-invalid batches, and mixed valid/invalid batches with
  stable per-item indexes. `SubTaskRequest::new` uses the same defaults as
  Python's dataclass, and `SubTaskOutcome::to_dict` emits the Python payload
  shape for callers that serialize sub-agent results. A Python-style active
