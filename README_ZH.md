...
live 套件会验证直接 runtime 完成、SDK 完成、`ask_user` 等待用户流程、workspace `write_file`/`read_file`、`file_str_replace`、`list_files`、`workspace_grep` 和 `file_info` 工具调用、后台命令 handoff，以及通过 `create_sub_task` 调用配置好的子 Agent：
