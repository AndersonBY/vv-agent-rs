...
- Python 风格 SDK session continuation 基础能力：`AgentSession::follow_up` 会在 completed run 后自动追跑 queued follow-up，`steer` 在 `continue_run(None)` 中优先于 follow-up，`clear_queues` 可清空待处理 prompt，`query` 会返回最终回答或带具体状态的错误，例如 `status=wait_user`。
