...
对 Python 实现的更深层 parity 仍待继续补齐，包括完整 sub-agent runtime 自动注册、distributed backends 和剩余内置工具。
