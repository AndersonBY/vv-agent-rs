...
对 Python 实现的更深层 parity 仍待继续补齐，主要是生产级 distributed-worker integration。Provider 请求序列化会统一交给 crates.io 官方 `vv-llm` crate；如果某些请求侧 provider 细节尚未被 `vv-llm` typed API 表达，应优先补到 `vv-llm`，不要在本仓库重新手写 provider 转换。
