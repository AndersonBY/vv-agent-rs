...
- SDK one-shot run 现在不再必须预先传入 runtime：默认会根据 `AgentSDKOptions.settings_file` 构造 `vv-llm` backed runtime；测试和嵌入方也可以注入 `LlmBuilder` 使用确定性 client。`AgentSDKOptions.runtime_hooks`、`log_handler`、`tool_registry_factory` 和 `execution_backend` 会同时作用于 SDK 自动构建和外部注入的 runtime，对齐 Python SDK 的扩展点。SDK 自动构建的 runtime 会把 vv-llm resolved token limits 写入 `model_context_window` 和 `reserved_output_tokens` metadata，但调用方显式传入的同名 metadata 优先。
