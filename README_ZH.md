...
对 Python 实现的更深层 parity 仍待继续补齐，包括完整 sub-agent 自动注册 / 事件 snapshot parity、distributed backends 和剩余内置工具。
