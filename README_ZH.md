...
- `runtime/` 已拆成 background sessions、captured processes、cancellation、hooks、shell resolution、state stores、`engine.rs` 主运行时、cycle-runner retry helper、tool-call runner helper、工具结果解析、sub-agent 执行、sub-task manager，以及 Python 风格 `runtime/backends/` 子模块（base / inline / thread / celery / celery_tasks），让后续继续补齐 Python parity 时改动更集中；crate 顶层仍保留稳定 re-export 兼容既有调用方，但源码不再把这些 runtime 模块平铺到根目录。`CycleRunner` 和 `ToolCallRunner` 也已像 Python 一样作为公开 runtime helper 导出，嵌入方可以只执行单轮 LLM planning 或单批工具调用，而不必走完整 `AgentRuntime`；`runtime::{Checkpoint, InMemoryStateStore, StateStore}` 和 `runtime::engine` 下的 sub-agent session helper 也对齐了 Python 的直接 runtime 导入路径；`MAX_PTL_RETRIES` 也作为 Python 风格 prompt-too-long retry 常量别名暴露。
