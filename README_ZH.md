...
- 已补 Python 风格 runtime token usage helper：可归一化 provider 原始 usage payload 中 prompt/completion 与 input/output 命名差异，保留 raw usage，并汇总逐 cycle token totals。`vv-llm` backed client 在 provider 未返回 usage 时也会估算 prompt/completion usage，避免 runtime 计费汇总和 memory compaction 启发式拿到全 0。
...
- 内置控制工具（`task_finish`、`ask_user`、`todo_write`），其中 TODO 管理已对齐 Python 风格的 payload 校验、自动 id、status / priority 默认值和 timestamp 保留；核心 workspace 工具（`list_files`、`file_info`、`read_file`、`write_file`、`file_str_replace`、`workspace_grep`、`read_image`，且 image message 注入仅限 `native_multimodal` 任务；`list_files` 也已补 Python 风格隐藏文件过滤和 scan-limit 估算 payload；`workspace_grep` 已补真正正则搜索，content 已改为文本摘要，结构化 matches 保留在 metadata，并补齐 Python 风格结构化 payload 上限；grep 指向单个文件时也会像 Python 一样绕过隐藏/忽略目录过滤）；通过 `compress_memory` 记录 memory notes；以及支持捕获输出、Python 风格 replacement 解码、stdin、通过 `bash_shell` metadata 选择 shell、前台超时转后台、后台轮询和后台命令终态 listener 自动通知的 `bash` / `check_background_command` 命令工具。
- 与 Python 一致的 workspace 路径安全策略：`LocalWorkspaceBackend` 默认拒绝访问 workspace 外路径，文件、图片、grep 和 bash 工具仍支持可信任务通过 metadata 显式放行。Tool context 会合并 `ExecutionContext.metadata` 和 task metadata，并保持 task metadata 优先级更高，对齐 Python runtime integration 行为。
